...
void
log_init_out(log_Level const level, FILE * const out)
{
...
void
log_init_err(log_Level const level, FILE * const err)
{
...
void
log_init_out_err(log_Level const level, FILE * const out, FILE * const err)
{
	assert(out && "out is NULL");
	assert(err && "err is NULL");
	if (!g_out) {
